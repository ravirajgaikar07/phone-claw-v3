...
Bundled skills (those shipped with PhoneClaw — code-helper, research,
...
def install_skill(source, name=None):
    ...
    tmpdir = tempfile.mkdtemp(prefix="phoneclaw-skill-")
    try:
...
def remove_skill(name):
    ...
    if name in _BUNDLED:
        return False, f"ERROR: '{name}' is bundled with PhoneClaw and can't be removed"
