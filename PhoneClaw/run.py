...
def _start_dream_timer():
    ...
    import threading
    from memory.dreaming import dream_cycle

    interval = config.DREAM_INTERVAL_HOURS * 3600
    ...
    t.daemon = True
    t.start()
    log.info("Dream timer started (every %dh, first in 5m)", config.DREAM_INTERVAL_HOURS)
